<template>
  <div v-if="!rubricaAVer" class="max-w-5xl mx-auto p-8 bg-white border border-gray-200 rounded-lg shadow space-y-8">
    <div class="flex items-center justify-between">
      <h2 class="text-2xl font-semibold text-gray-800">{{ $t('gestioRubriques') }}</h2>
      <button
      ...
      <ul class="divide-y divide-gray-200">
        ...
        >
          <div>
            ...
            <p class="text-gray-600 text-sm">{{ $t('rol') }}: {{ $t(rubrica.rol.nombre.toLowerCase() )}}</p>
          </div>
    ...
    <ConfirmacionEliminacion
      :mensaje="$t('mensajeConfirmacionEliminar')"
  ...
  methods: {
    ...
    async eliminarRubrica() {
      try {
        ...
        toast.error(this.$t('errorEliminarRubrica'))
      }
    },
    verMas(rubrica) {
